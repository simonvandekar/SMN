...
## Using `easythresh` with `pbj` output

Applying this analysis to the pbj GLM output.
...
Two tables of the output from `easythresh` using FSL analysis and `pbj` analysis as input. Results are for the diagnostic effect. They should be the same.
...
rdsDiamond = file.path(analysisDir, 'dxGLM', 'pbjBoot.rds')
```
...
set.seed(123)
nboot=500
status = pbjInference(abideDXglm, nboot=nboot, rdata_rds=rds, cft_p=0.01)
```
...
set.seed(123)
nboot=500
status = pbjInference(abideDXglm, nboot=nboot, rdata_rds=rdsDiamond, statistic=cluster, cft_p=0.01, kernel='diamond')
```

```{r, fig.cap="Cluster extent inference results from the parametric bootstrap procedure in the pbj package.", eval=FALSE}
# load in results
abideDXglm = readRDS(rds)
abideDXglm
image(abideDXdiamond, cft_p=0.01)
knitr::kable(table.statMap(abideDXglm, cft_p=0.01)[1:10,], caption = "Ten largest clusters from the parametric bootstrap procedure applied to OLS model in the pbj package.")
#inferenceIndex(abideDX$pbj$obsStat, method='cei')
```

```{r, fig.cap="Cluster extent inference results from the parametric bootstrap procedure in the pbj package.", eval=FALSE}
# load in results
abideDXdiamond = readRDS(rdsDiamond)
abideDXdiamond
image(abideDXdiamond, cft_p=0.01)
knitr::kable(table.statMap(abideDXdiamond, cft_p=0.01)[1:10,], caption = "Ten largest clusters from the parametric bootstrap procedure applied to OLS model in the pbj package.")
#inferenceIndex(abideDX$pbj$obsStat, method='cei')
```
...
This uses the "robust" covariance approach.
...
set.seed(123)
nboot=500
status = pbjInference(abideDX, nboot=nboot, rdata_rds=rds, cft_p=0.01)
```
...
abideDX = readRDS(rds)
abideDX
image(abideDX, cft_p=0.01)
knitr::kable(table.statMap(abideDX, cft_p=0.01)[1:10,], caption= "Ten largest clusters from the semiparametric bootstrap procedure applied to robust model in the pbj package.")
#inferenceIndex(abideDX$pbj$obsStat, method='cei')
```
